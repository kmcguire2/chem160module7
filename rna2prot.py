...
protein = "" #empty string to hold the amino acid sequence

while len(rna) >= 3: #continue until wee are out of codons
    seq = rna[0:3] #first 3 letters in RNA
    rna = rna[3:] #remove first 3 letters
    if seq in code: # check if sequence is a key in the dictionary
        if code[seq] == "STOP": #check for stop codons
