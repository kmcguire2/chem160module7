...
# parse through each line that is read in
    if line.find("pH:") >= 0:  # if the word is found, return position of string
        pH_list = line.split()   # parse line
        date = pH_list[0].split("/")  # parse out date
        for i in range(3):
            date[i] = int(date[i])  # convert values in date to integer
        pH_value = float(pH_list[2])  # convert pH to a float
    ...
    #if line begins with alert, add to the number of alerts
